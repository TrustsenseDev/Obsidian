...
local SaveManager = {} do
    ...
    function SaveManager:GetAutoOverwriteEnabled()
        SaveManager:CheckFolderTree()

        local autoOverwritePath = self.Folder .. "/settings/autooverwrite.txt"
        if SaveManager:CheckSubFolder(true) then
            autoOverwritePath = self.Folder .. "/settings/" .. self.SubFolder .. "/autooverwrite.txt"
        end

        if isfile(autoOverwritePath) then
            local successRead, enabled = pcall(readfile, autoOverwritePath)
            if not successRead then
                return false
            end
    ...
    function SaveManager:SaveAutoOverwriteConfig(enabled)
        SaveManager:CheckFolderTree()
        ...
        local success = pcall(writefile, autoOverwritePath, tostring(enabled))
        if not success then return false, "write file error" end

        return true, ""
    end

    function SaveManager:DeleteAutoOverwriteConfig()
        SaveManager:CheckFolderTree()
        ...
        local success = pcall(delfile, autoOverwritePath)
        if not success then return false, "delete file error" end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddToggle("SaveManager_AutoOverwrite", {
            Text = "Auto overwrite selected config",
            Default = self:GetAutoOverwriteEnabled(),
            Callback = function(Value)
                local success, err = self:SaveAutoOverwriteConfig(Value)
                if not success then
                    return self.Library:Notify("Failed to set auto overwrite: " .. err)
                end

                if Value then
                    self.Library:Notify("Auto overwrite enabled")
                else
                    self.Library:Notify("Auto overwrite disabled")
                end
            end
        })

        -- Store original callback functions to hook into them
        self._originalCallbacks = {}

        local function handleAutoOverwrite()
            if not self.Library.Options.SaveManager_AutoOverwrite.Value then return end

            local selectedConfig = self.Library.Options.SaveManager_ConfigList.Value
            if not selectedConfig or selectedConfig == "" then return end

            local success, err = self:Save(selectedConfig)
            if not success then
                self.Library:Notify("Failed to auto overwrite config: " .. err)
            end
        end

        -- Hook into all existing toggles and options
        for idx, toggle in pairs(self.Library.Toggles) do
            if not self.Ignore[idx] and not idx:find("SaveManager_") then
                if not self._originalCallbacks[idx] then
                    self._originalCallbacks[idx] = toggle.Callback

                    toggle.Callback = function(Value)
                        if self._originalCallbacks[idx] then
                            self._originalCallbacks[idx](Value)
                        end
                        handleAutoOverwrite()
                    end
                end
        ...
        for idx, option in pairs(self.Library.Options) do
            if not self.Ignore[idx] and not idx:find("SaveManager_") then
                if not self._originalCallbacks[idx] then
                    self._originalCallbacks[idx] = option.Callback
        ...
        self:SetIgnoreIndexes({ "SaveManager_ConfigList", "SaveManager_ConfigName", "SaveManager_AutoOverwrite" })
    end
