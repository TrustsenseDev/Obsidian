...
do
	...
	--// Auto Save \\--
	function SaveManager:SetAutoSave(enabled, interval, configName)
		...
		if configName and type(configName) == "string" and configName ~= "" then
	...
	function SaveManager:StartAutoSave()
		...
		self._autoSaveConnection = game:GetService("RunService").Heartbeat:Connect(function()
			if not self._lastAutoSave then
				self._lastAutoSave = tick()
				return
			end

			if tick() - self._lastAutoSave >= self.AutoSaveInterval then
				self:Save(self.AutoSaveConfig)
				self._lastAutoSave = tick()

				if self.Library then
					self.Library:Notify(string.format("Auto-saved config %q", self.AutoSaveConfig))
				end
			end
		end)
	...
	function SaveManager:BuildConfigSection(tab)
		...
		section:AddSlider("SaveManager_AutoSaveInterval", {
			...
			Rounding = 0,
			Callback = function(value)
				self:SetAutoSave(self.AutoSaveEnabled, value, nil)
			end,
		})

		section:AddInput("SaveManager_AutoSaveConfig", {
			Text = "Auto-Save Config Name",
			Default = self.AutoSaveConfig,
			Callback = function(value)
				if value and value:gsub(" ", "") ~= "" then
					self:SetAutoSave(self.AutoSaveEnabled, nil, value)
				end
			end,
		})
